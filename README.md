...
| 👤 Local benchmarking profiles | 📇 Per-user model registry + ONNX validator | 🧪 Dice · IoU · HD95 · ASSD metrics | 📊 Model comparison + CSV/JSON export |
| 🩻 Segmentation · classification · detection | 📈 ROC · PR · calibration · FROC · Bland-Altman | 🗂️ NIfTI · DICOM-SEG · RTSTRUCT · CSV refs | 🔒 CI-enforced no-upload invariant |

> 🔬 **Pathology mode lands in v0.6.0.** Whole-slide image viewer (OpenSeadragon-based pyramidal viewer · OME-TIFF · Aperio SVS · Hamamatsu NDPI · TIFF · PNG · JPEG), tile-based ONNX inference (segmentation / classification / heatmap / detection), MPP-aware distance ruler, six-colour brush + eraser with per-colour PNG export. Full reference: [`docs/PATHOLOGY.md`](docs/PATHOLOGY.md). Roadmap: [`docs/ROADMAP.md#v060--pathology-mode`](docs/ROADMAP.md#v060--pathology-mode).
>
...
>
> 🫁 **TotalSegmentator (whole-body anatomical segmentation) lands in v0.7.4 → v0.7.7** under the **On-Prem AI** sidebar section. Two runtime paths: (a) **native Python sidecar** — Tauri desktop spawns your local `totalsegmentator` install (Mac MPS / CPU / Linux CUDA — whatever your PyTorch resolves to), streams stdout/stderr live, captures the per-class NIfTI masks; (b) **BYO ONNX URL** — community-converted exports run via the same browser ORT stack as SAM. Full setup + task list + troubleshooting: [`docs/TOTALSEGMENTATOR.md`](docs/TOTALSEGMENTATOR.md).
>
> 🧪 **Radiology benchmarking (per-user, local) lands in v0.11.0** under the new **Benchmark** sidebar section + a header profile control (`👤`). Log into a **local profile** (name + optional passphrase — no backend, no upload, no accounts; benchmarks are isolated per profile on your device), register your own ONNX models (the manifest + ONNX graph are validated — opset / IO / loadability), capture a reference (a loaded ground-truth mask or a prior model's output), and **score the current inference result**: Dice, IoU/Jaccard, HD95, ASSD, precision/recall, volume difference — spacing-aware and grid-aligned. Compare models side-by-side and export CSV/JSON. A task-agnostic classification metric set (AUROC/AUPRC/sensitivity/specificity/Brier/ECE) ships in the engine for the CXR fast-follow. Adapts the ACR **Assess-AI** field schemas. Roadmap (Phases 2–4: DICOM-SEG/RTSTRUCT import, completeness/concordance dashboards, cohorts, governance): [`docs/benchmark/ROADMAP.md`](docs/benchmark/ROADMAP.md).
>
> 🧪 **v0.12 completes the benchmarking roadmap + full source-doc coverage.** Three tasks — **segmentation** (Dice/IoU/HD95/ASSD), **classification** (import a labels CSV/JSON → threshold slider → ROC/PR/calibration/confusion) and **detection** (import boxes → IoU threshold → lesion sensitivity/FP-per-image/mAP/**FROC**). Reference-label import from **NIfTI masks, DICOM-SEG, RTSTRUCT, CSV/JSON labels, bounding boxes**. Assess-AI-style **completeness + concordance** (Wilson 95% CI) with discordant-case **adjudication**, **subgroup** metrics + forest plots, **Bland-Altman** volume agreement, **model registry** with ONNX shape/precision/operator extraction, **model cards**, versioned **benchmark definitions**, and **reference-set locking** (content fingerprint). HD95/ASSD run in a Web Worker. Everything stays on-device: a **CI-enforced [on-premise invariant](docs/benchmark/loop.md)** fails the build if any benchmarking module ever introduces a network call. Full doc→feature map: [`docs/benchmark/GAP-ANALYSIS.md`](docs/benchmark/GAP-ANALYSIS.md).
>
...
- **[examples/](examples/)** — where to get real sample data and models (in-app: Examples card → benchmark kits and public sample scans)
